    ...
    request(uri, (error, response, body) => {
      if (!error && response.statusCode == 200) {
        ...
        if (json.errcode == undefined || json.errcode == 0) {
          ...
          console.log('get token remote returns error: ' + JSON.stringify(json));
          next(-1, '');
        }
      } else {
        console.log('error get token remote: ' + error);
        next(-1, '');
      }
    });
  }
    ...
    request(uri, (error, response, body) => {
      if (!error && response.statusCode == 200) {
        ...
        if (json.errcode == undefined || json.errcode == 0) {
          ...
          console.log('get ticket remote returns error: ' + JSON.stringify(json));
          next(-1, '');
        }
      } else {
        console.log('error get ticket remote: ' + error);
        next(-1, '');
      };
    });
  }
...
// (url, next) => next(json data)
wx.get_signed_data = (url, next) => {
  wx.get_ticket((err, ticket) => {
    wx.get_app_id((err, app_id) => {
      var nonce = tool.nonce();
      var json = {
        'noncestr': nonce,
        'jsapi_ticket': ticket,
